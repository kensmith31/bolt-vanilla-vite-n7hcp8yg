/*
  ...
  1. Changes
    ...
    - Fix depreciation and holdback status changes
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    -- Check if depreciation_amount is not null
    IF NEW.depreciation_amount IS NOT NULL THEN
      NEW.status = 'adjusted'::item_status;
    END IF;

    -- Check if holdback_due is greater than 0
    IF NEW.holdback_due > 0 THEN
...
-- Update existing items to correct status
UPDATE items
SET updated_at = NOW()
WHERE true;