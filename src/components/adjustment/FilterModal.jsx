...
export function FilterModal({ isOpen, onClose, claimId }) {
  ...
  async function fetchFilterData() {
    try {
      ...
      const uniqueRooms = [...new Set(items.map(item => item.room))].filter(Boolean).sort();
      setRooms(uniqueRooms);

      const uniqueStatuses = [...new Set(items.map(item => item.status))].filter(Boolean).sort();
      ...
      const uniqueTaxRates = [...new Set(items.map(item => item.tax_rate))].filter(Boolean)
        .map(rate => (rate * 100).toFixed(3))
        .sort((a, b) => parseFloat(a) - parseFloat(b));
      setAvailableTaxRates(uniqueTaxRates);

      const uniqueDepreciationRates = [...new Set(items.map(item => item.depreciation_percent))]
        .filter(Boolean)
        .map(rate => (rate * 100).toFixed(0))
        .sort((a, b) => parseFloat(a) - parseFloat(b));
      setAvailableDepreciationRates(uniqueDepreciationRates);

      const uniqueAges = [...new Set(items.map(item => item.age))].filter(Boolean)
        .sort((a, b) => a - b);
        ...
        <Dialog.Panel className="w-full max-w-2xl rounded-lg bg-white p-6 shadow-xl">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                <h3 className="font-medium text-gray-700 mb-3">Basic Filters</h3>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.value === 'true') {
                          ...
                          const rcaSelect = document.querySelector('select[name="replacement_cost_applies"]');
                          if (rcaSelect) {
