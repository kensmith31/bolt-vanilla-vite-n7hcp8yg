...
import { PlusIcon, PencilIcon, UserPlusIcon, UserMinusIcon, ChevronDownIcon, ChevronUpIcon } from '@heroicons/react/24/outline';
...
export function ClaimList() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!currentUser || !['admin', 'desk_adjuster', 'field_adjuster'].includes(currentUser.roles?.name)) {
      ...
      toast.error(`Failed to ${editingClaim ? 'update' : 'create'} claim: ${err.message}`);
    }
  };
  ...
  const canCreateClaim = currentUser && ['admin', 'desk_adjuster', 'field_adjuster'].includes(currentUser.role);
              ...
              {claims.map((claim) => (
                <tr key={claim.file_number} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      >
                        <span className="text-xl font-bold text-blue-700">#{claim.file_number}</span>
                      </Link>
                    </td>
                          ...
                          <div key={participant.id} className="flex items-center gap-2">
                            <span>
                              ...
                              <span className="text-xs text-gray-500 ml-1">
                                ({participant.roles && participant.roles.name ? participant.roles.name.replace('_', ' ') : 'No Role'})
                              </span>
                            </span>
                            <button
                              onClick={() => handleRemoveParticipant(claim.file_number, participant.id)}
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setFormData({ ...formData, property_zip_code: e.target.value })}
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex items-center">
                        <input
                          ...
                          onChange={(e) => setFormData({ ...formData, depreciation_applicable: e.target.checked })}
                        />
                        <label htmlFor="depreciation_applicable" className="ml-2 block text-sm text-gray-700">
                      ...
                      <div className="flex items-center">
                        <input
                          ...
                          onChange={(e) => setFormData({ ...formData, depreciation_recoverable: e.target.checked })}
                        />
                        <label htmlFor="depreciation_recoverable" className="ml-2 block text-sm text-gray-700">
                  ...
                  <form onSubmit={handleAddParticipant} className="space-y-4">
                    <div>
                      ...
                      >
                        ...
                        {users
                          .filter(user => !editingClaim?.claim_participants?.some(
                            participant => participant.user_id === user.id
                          ))
                          .map((user) => (
                            <option key={user.id} value={user.id}>
                              {user.first_name} {user.last_name} ({user.email}) - {user.roles && user.roles.name ? user.roles.name.replace('_', ' ') : 'No Role'}
                            </option>
                          ))}
                      </select>
                    </div>
